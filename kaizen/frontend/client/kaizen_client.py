...
class KaizenClient:
    """Wrapper client around kaizen cortex backends."""
    ...
    def __init__(self, config: KaizenConfig | None = None):
        ...
        if self.config.backend == 'milvus':
            from kaizen.backend.milvus import MilvusCortexBackend
            self.backend = MilvusCortexBackend(self.config.settings)
        elif self.config.backend == 'filesystem':
            from kaizen.backend.filesystem import FilesystemCortexBackend
            self.backend = FilesystemCortexBackend(self.config.settings)
        else:
            raise NotImplementedError(f'Cortex backend not implemented: {self.config.backend}')
