...
- The current task explicitly asks you to accept or approve Venmo payment requests.
- The requests are received by the user and are pending.
- The target senders are selected by contact relationships such as friend or coworker.

## Does Not Apply When

- The task asks to remind someone about a payment request. Use the payment-request and contact-filtering pattern only, then inspect API docs for the reminder endpoint.
- The task asks to deny, create, update, delete, or answer questions about payment requests.
- The task concerns Venmo transactions or the social feed rather than payment requests.
