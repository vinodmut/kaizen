...
- The current task explicitly asks you to like Venmo transactions.
- The target transactions come from Venmo social feed or transaction data.
- The target set is defined by a contact relationship such as sibling, parent, friend, or coworker.
- The target set has a time filter such as yesterday, today, a date, or the ongoing year.

## Does Not Apply When

- The task asks for an answer only, such as a count, total, amount, or name. In that case, reuse only the data-gathering pattern and do not call `venmo.like_transaction`.
- The task asks to remind, accept, approve, deny, send, create, update, or delete a payment request.
- The task is for a non-Venmo app.
- The only overlap is a generic AppWorld rule already present in the task prompt, such as checking API docs or paginating results.
