...
│   ├── <slug>__<eid>.md                      ← audit-only observation; `eid` matches `ev:<id>` frontmatter
│   └── _id_index.json                        ← evidence id → relpath
├── summaries/
│   ├── index.md                              ← section index (regenerated by catalog)
...
guideline/cluster/skill/task/subtask page, plus audit-only evidence rows.
The schema:

```json
{
  "kind": "guideline" | "cluster" | "skill" | "task" | "subtask" | "evidence",
  "id": "<12-hex-char content hash, OR cluster:<slug>, OR skill:<slug>, OR task:<slug>, OR subtask:<slug>, OR ev:<id>>",
  "title": "<short title>",
  "tags": ["...", "..."],
  ...
  "recall_preferred": false,  // evidence rows only
...
tasks, then audit evidence**. Cluster pages are *aggregators* — when a cluster matches your
...
**Evidence** (`kind: "evidence"`) lives under `<wiki>/evidence/`.
Evidence rows are provenance and audit material for how guidelines and
skills were generated. They are not direct advice. Ignore evidence rows
for normal recall unless you are auditing a guideline or checking whether
a proposed wiki update is grounded in trajectory-visible behavior.
